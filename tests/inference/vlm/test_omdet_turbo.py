"""Tests for the OmDet-Turbo inferencer with mocked transformers/torch.
...
from object_detection_eval.inference.vlm.omdet_turbo import (  # noqa: E402
...
def _mock_transformers():
    ...
    with (
        patch("object_detection_eval.inference.vlm.omdet_turbo.AutoProcessor") as mock_proc_cls,
        patch(
            "object_detection_eval.inference.vlm.omdet_turbo.AutoModelForZeroShotObjectDetection"
        ) as mock_model_cls,
        patch("object_detection_eval.inference.vlm.omdet_turbo.torch") as mock_torch,
    ):
