"""YOLO26 detector: centered letterbox (pad 114), RGB, /255 (CORE-07).

Composes the shared `Letterbox`/`LetterboxTransform` (Plan 05) with
`YOLO26PostProcessor` (Plan 06) behind `ONNXInferencer`. `predict()` is
...
This exists to fix the train/eval preprocessing mismatch documented in the
source repo: YOLO26 is trained with letterbox but was previously evaluated
with a plain square resize, which collapses non-square frames and costs a
large amount of mAP.
"""
...
from object_detection_eval.inference.postprocess import YOLO26PostProcessor
...
class YOLO26Detector(ONNXInferencer):
    """YOLO26 ONNX detector using Ultralytics-style centered letterbox preprocessing.
    ...
    def __init__(
        ...
        post_processor = YOLO26PostProcessor(label_map, confidence_threshold=confidence_threshold)
        super().__init__(
        ...
        self._letterbox = Letterbox(LetterboxConfig.yolo26())
