...
- [x] 04-01-PLAN.md — run_benchmark.py reproduces the 7-model @640 table within tolerance + rank order; mirror the last local-only YOLOX-M @640 ONNX to GCS (REPRO-01) [wave 1]
- [x] 04-02-PLAN.md — COCO reference: YOLOX-S on val2017 reproduces ~39.6 vs 40.5, confirming the supervision-vs-pycocotools gap (REPRO-02) [wave 1]
- [x] 04-03-PLAN.md — reproduce the paired-bootstrap CIs + the YOLOX-M/YOLO26m joint-best tie; correct the methodology drift narrative (REPRO-03) [wave 2, depends 04-01]
...
- [x] 06-01-PLAN.md — LAT-01 uniform e2e latency harness (run_latency.py + manifest + LAT-04 band helpers), CPU-developable [Wave 1]
- [x] 06-02-PLAN.md — LAT-03 EfficientNMS_TRT graft + RTMDet TopK strip + pixi graphsurgeon/trt envs + pinned [trt] extra, CPU-developable [Wave 1]
- [x] 06-03-PLAN.md — LAT-02 trtexec fp16 build/benchmark + full EfficientNMS graft matrix + LAT-04 honest-label verdict, T4 checkpoint [Wave 2, depends on 06-01 + 06-02]
...
- [ ] 07-01-PLAN.md — Close the accuracy-data gap: --write-results on run_benchmark.py + run_bootstrap_gate.py, run + commit the 3 accuracy JSONs (REPORT-01) [wave 1]
- [ ] 07-02-PLAN.md — Torch-free report/ generator (loaders + renderers + inject) + generate_report.py --check/--write + golden tests (REPORT-01) [wave 1]
- [ ] 07-03-PLAN.md — FINAL_COMPARISON_640.md: preprocessing lede, 7-model table + CIs + per-class AP, fairness audit, honest-labeled latency (REPORT-02) [wave 2, depends 07-01, 07-02]
- [ ] 07-04-PLAN.md — VLM_VS_FINETUNED.md + docs/methodology.md + README.md reproduction path (REPORT-03, REPORT-04, REPORT-05) [wave 2, depends 07-01, 07-02]
...
| 7. Reports & Docs | 0/4 | Not started | - |
